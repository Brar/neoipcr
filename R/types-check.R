...
check_neoipcr_dept_ds <- function(x, require_scalar = TRUE, allow_null = FALSE) {
  if (!missing(x)) {
    if (require_scalar && is_scalar_neoipcr_dept_ds(x)) return(invisible(NULL))
    if (is_neoipcr_dept_ds(x)) return(invisible(NULL))
    if (allow_null && is_null(x)) return(invisible(NULL))
  }
  stop_input_type(x, "a neoipcr_dept_ds object")
}
