...
|------|---------|
| **Import pipeline** | |
| `R/import-dhis2.R` | `import_dhis2()` orchestrator + cross-cutting utilities (`add_key_column`, `convert_value`, `value_type_family()` + `spread_typed_values()` — DHIS2 value-type families → typed `value_*` columns, `dhis2_ou_dialect()` version→org-unit-request dialect, `neoipcr_supported_versions()` + the unsupported-version warning helpers) |
...
| `R/dhis2-metadata-orgunits.R` | Org unit request builder + `read_organisationUnits*` response readers + the custom-attribute value split (`read_organisationUnit_attribute_values`) and resolver (`resolve_organisationUnit_attribute_values`) |
...
A department counts as a test unit when it is a member of the `TEST_UNITS` org-unit group **or** carries the `IsTestunit` custom attribute (`TRUE_ONLY`) itself. The attribute is read on every import, whatever `include_custom_attributes` says (see Org-unit Attributes below). A test flag on an ancestor — a hospital's group membership or attribute — is not consulted.
...
|---|---|---|
| `metadata$orgUnitAttributes` | `code`, `name`, `valueType` (the DHIS2 enum name, as character) | any opted-in entity is present (`include_<entity>` not `"no"`) |
...
A closed gate yields a 0×0 tibble; there is no "pseudo" tier, because the opt-in is the whole contract. The values can carry personal data (a site's contact person), so opting in under a pseudonymized entity re-identifies it — the caller's explicit choice, as with `include_dhis2_ids`. Without the opt-in no value other than `IsTestunit` is resolved at all, so a malformed value can only ever be reported to a caller who receives the values.

**Typed values.** Each value row fills exactly one typed column, chosen by the attribute's value type through the families DHIS2 declares in `ValueType.java` (`value_type_family()` in `R/import-dhis2.R`):
...
**Resolution by code.** The definitions (`/api/metadata` `attributes`, filtered to `organisationUnitAttribute:eq:true`) are requested on every import, and a value's attribute UID is resolved to `attribute_code` through them; no UID is public on any of the three tibbles. A value whose definition the caller cannot read (the contact-person attributes are shared privately) is dropped and counted in a debug log line. Department values are always requested because of `IsTestunit`; hospital values only when opted in. Values are resolved for the org units that survived the metadata narrowing, so a pruned department's value never raises a warning.

**`IsTestunit`.** Read on every import regardless of the opt-in, folded into `isTest` (see DHIS2 Test Units above) and dropped from the values tables. `assert_data_protection()` aborts if an `IsTestunit` row survives, or if a values table is populated without the opt-in.
...
| `tests/testthat/test-dhis2-metadata-orgunits.R` | The `/organisationUnits` request shape (the always-on department `attributeValues` fragment, the opt-in hospital fragment, the always-on `attributes:` definitions request) and the org-unit attribute path: value split, hospital dedup with the list column, resolver (UID → code, undefined dropped, non-surviving parents dropped), definitions reader |
