...
trial’s per-site dates, for example) are imported on request:
`dhis2_dataset_options(include_custom_attributes = "departments")` adds
their typed values as `metadata$departmentAttributeValues`, with the
